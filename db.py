...
async def history_diff(sig_id: str):
    """Return latest and previous history rows and value differences."""
    current = await latest_history(sig_id)
    if not current:
        return None
    previous = await previous_history(sig_id, current["ts"])
    diff = {}
    if previous:
        fields = [
            "drawdown",
        ...
        for f in fields:
            c = current.get(f)
            p = previous.get(f)
            diff[f] = c - p if c is not None and p is not None else None
